...
//#define SEND_MSG "poweroff"
int ExternalPoweroff(void) {
    int fd;                            //文件描述符
    int ret;                           //返回调用函数的状态
    int len;
    char send_buf[] = {0xAA, 0xBB, 0xCC, 0xDD};
    fd = UART0_Open(SERIAL_PORT); //打开串口，返回文件描述符
    if (!fd)
        return FALSE;
    ret = UART0_Init(fd, SPEED, 0, 8, 1, 'N');
    if (!ret) {
        printf("UART0_Init failed!ret=%d\n", ret);
        UART0_Close(fd);
        return FALSE;
    }
